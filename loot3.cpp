...
int get_max_index1(vector<int> weights, vector<int> values) {
    int max_i = 0;
    double max = 0;
...
double get_optimal_value1(int capacity, vector<int> weights, vector<int> values) {
    ...
    for (int i = 0; i < weights.size(); i++) {
        ...
        int index = get_max_index1(weights, values);
...
int main() {
    ...
    double optimal_value = get_optimal_value1(capacity, weights, values);
